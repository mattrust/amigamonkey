/****************************************************************************
** Meta object code from reading C++ file 'findinfilesdialog.h'
**
** Created: Mon 20. Jul 15:25:46 2015
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../ted/findinfilesdialog.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'findinfilesdialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_FindInFilesDialog[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      35,   19,   18,   18, 0x05,

 // slots: signature, parameters, type, tag, flags
      61,   18,   18,   18, 0x0a,
      68,   18,   18,   18, 0x0a,
      77,   18,   18,   18, 0x0a,
      97,   92,   18,   18, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_FindInFilesDialog[] = {
    "FindInFilesDialog\0\0path,pos,length\0"
    "showCode(QString,int,int)\0find()\0"
    "cancel()\0browseForDir()\0item\0"
    "showResult(QListWidgetItem*)\0"
};

void FindInFilesDialog::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        FindInFilesDialog *_t = static_cast<FindInFilesDialog *>(_o);
        switch (_id) {
        case 0: _t->showCode((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2])),(*reinterpret_cast< int(*)>(_a[3]))); break;
        case 1: _t->find(); break;
        case 2: _t->cancel(); break;
        case 3: _t->browseForDir(); break;
        case 4: _t->showResult((*reinterpret_cast< QListWidgetItem*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData FindInFilesDialog::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject FindInFilesDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_FindInFilesDialog,
      qt_meta_data_FindInFilesDialog, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &FindInFilesDialog::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *FindInFilesDialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *FindInFilesDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_FindInFilesDialog))
        return static_cast<void*>(const_cast< FindInFilesDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int FindInFilesDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void FindInFilesDialog::showCode(const QString & _t1, int _t2, int _t3)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)), const_cast<void*>(reinterpret_cast<const void*>(&_t3)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
