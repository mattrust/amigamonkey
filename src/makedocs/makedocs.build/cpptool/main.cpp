...
class c_George : public Object,public virtual c_ILinkResolver,public virtual c_IPrettifier{
	...
	String m_errinfo;
	c_StringMap4* m_pages;
	c_StringMap6* m_indexcats;
	c_StringMap4* m_content;
	String m_docbase;
	int m_inrem;
	c_Toker* m_ptoker;
	c_George();
	...
	c_StringMap4* p_GetIndex(String);
	...
	static String m_MonkeyKeywords;
...
class c_ApiDoccer : public Object,public virtual c_ILinkResolver{
	...
	void p_ParseMonkeydocFile(String,String);
	void p_AddDocsToDecl(c_StringMap7*,c_Decl2*);
	void p_ParseMonkeyFile(String,String);
...
class c_ScopeDecl : public c_Decl2{
	public:
	c_StringMap2* m_declsByUident;
	c_Stack3* m_decls;
	c_StringMap3* m_declsByKind;
...
class c_Node3 : public Object{
	...
	c_Node3* m_parent;
	c_Node3();
	c_Node3* m_new(String,c_Decl2*,int,c_Node3*);
	c_Node3* m_new2();
	c_Node3* p_NextNode();
	c_Decl2* p_Value();
	void mark();
...
class c_Node4 : public Object{
	...
	c_Node4* m_parent;
	c_Node4();
	c_Node4* m_new(String,c_Stack3*,int,c_Node4*);
	c_Node4* m_new2();
	c_Node4* p_NextNode();
	c_Stack3* p_Value();
	String p_Key();
...
class c_Node7 : public Object{
	...
	c_Node7* m_right;
	c_Node7* m_left;
	c_StringMap4* m_value;
	int m_color;
	c_Node7* m_parent;
	c_Node7();
	c_Node7* m_new(String,c_StringMap4*,int,c_Node7*);
	c_Node7* m_new2();
	c_Node7* p_NextNode();
	String p_Key();
	c_StringMap4* p_Value();
	void mark();
};
...
class c_NodeEnumerator3 : public Object{
	...
	c_Node5* m_node;
	c_NodeEnumerator3();
	c_NodeEnumerator3* m_new(c_Node5*);
...
class c_NodeEnumerator5 : public Object{
	...
	c_Node4* m_node;
	c_NodeEnumerator5();
	c_NodeEnumerator5* m_new(c_Node4*);
...
class c_NodeEnumerator6 : public Object{
	...
	c_Node3* m_node;
	c_NodeEnumerator6();
	c_NodeEnumerator6* m_new(c_Node3*);
...
c_George::c_George(){
	...
	m_errinfo=String();
	m_pages=(new c_StringMap4)->m_new();
	m_indexcats=(new c_StringMap6)->m_new();
	m_content=(new c_StringMap4)->m_new();
	m_docbase=String();
	m_inrem=0;
...
c_StringMap4* c_George::p_GetIndex(String t_cat){
	c_StringMap4* t_index=m_indexcats->p_Get(t_cat);
	if(!((t_index)!=0)){
		t_index=(new c_StringMap4)->m_new();
...
void c_George::p_AddToIndex(String t_cat,String t_ident,String t_path){
	...
	c_StringMap4* t_index=p_GetIndex(t_cat);
	...
	t_index->p_Set4(t_uident,t_path);
}
...
void c_George::p_SetPageContent(String t_page,String t_html){
	m_content->p_Set4(t_page,t_html);
}
void c_George::p_MakeIndices(){
	...
	while(t_->p_HasNext()){
		...
		c_StringMap4* t_index=t_it->p_Value();
		...
		while(t_2->p_HasNext()){
			c_Node5* t_it2=t_2->p_NextObject();
			t_maker->p_AddItem();
...
String c_George::p_ResolveLink(String t_link,String t_text){
	if(t_link.StartsWith(String(L"#",1)) || t_link.StartsWith(String(L"http:",5)) || t_link.StartsWith(String(L"https:",6))){
...
void c_George::p_MakeDocs(){
	...
	while(t_6->p_HasNext()){
		c_Node5* t_it2=t_6->p_NextObject();
...
String c_George::m_MonkeyKeywords;
	...
	do{
		...
		if(t_1==1){
			...
			if(t_1==2){
				...
				if(t_1==3){
					t_cls=String(L"i",1);
					if(m_MonkeyKeywords.Contains(String(L";",1)+t_toke.ToLower()+String(L";",1))){
						t_cls=String(L"k",1);
					}
				}else{
...
bool c_ApiDoccer::p_LoadExample(c_Decl2* t_decl,String t_dir){
	...
	String t_src=LoadString(t_dir+String(L"/",1)+t_decl->m_ident+String(L"_example.monkey",15));
	if(!((t_src).Length()!=0)){
		return false;
	}
	t_decl->m_docs->p_Set4(String(L"example",7),t_src);
...
void c_ApiDoccer::p_EndSect(String t_sect,c_StringStack* t_docs,c_Decl2* t_doccing){
	if(!t_docs->p_IsEmpty()){
		...
		if((t_t).Length()!=0){
			t_doccing->m_docs->p_Set4(t_sect,t_t);
		}
		t_docs->p_Clear();
	}
}
void c_ApiDoccer::p_ParseMonkeydocFile(String t_srcpath,String t_modpath){
...
void c_ApiDoccer::p_AddDocsToDecl(c_StringMap7* t_docs,c_Decl2* t_decl){
	...
	while(t_->p_HasNext()){
		...
		t_decl->m_docs->p_Set4(t_it->p_Key(),t_it->p_Value()->p_Join(String(L"\n",1)));
	}
}
void c_ApiDoccer::p_ParseMonkeyFile(String t_srcpath,String t_modpath){
	m_george->p_SetErrInfo(t_srcpath);
	c_Parser* t_parser=(new c_Parser)->m_new(String());
	...
	while(t_2<t_.Length()){
		...
		if(t_parser->p_Toke()==String(L"#",1)){
			...
			if(t_22==String(L"rem",3)){
				if(t_parser->p_Bump()==String(L"monkeydoc",9)){
		...
		if((t_sect).Length()!=0){
			if(t_parser->p_TokeType()==3){
				String t_id2=t_parser->p_Toke();
				if(t_parser->p_Bump()==String(L":",1)){
					String t_3=t_id2.ToLower();
		...
		if(((t_line.Trim()).Length()!=0) && !((t_mdecl)!=0)){
			return;
		}
					...
					if(t_4==String(L"import",6)){
						if(t_pub && t_mdoc){
							...
							if((t_pdecl)!=0){
								...
								if(FileType(t_p+String(L".monkey",7))==1 || FileType(t_p+String(L"/",1)+t_p+String(L".monkey",7))==1){
									if(bb_os_StripDir(bb_os_ExtractDir(t_srcpath))==bb_os_StripAll(t_srcpath)){
										...
										if(t_modpath.Contains(String(L".",1))){
											String t_id3=bb_os_StripExt(t_modpath);
											if(t_id3.Contains(String(L".",1))){
												t_id3=bb_os_ExtractExt(t_id3);
											}
											if(t_id3!=t_ident){
												t_pdecl->m_ident=t_id3+String(L".",1)+t_ident;
											}
										}
									}
								}
...
void c_ApiDoccer::p_ParseModules(String t_dir,String t_modpath){
	...
	if(FileType(t_p)==1){
		m_scopeTemplate=(new c_PageMaker)->m_new(LoadString(t_p));
	}
	...
	while(t_2<t_.Length()){
		...
		if(t_7==2){
			if(t_f==String(L"3rdparty.monkeydoc",18)){
				...
				while(t_4<t_3.Length()){
					...
					if(t_8==2){
						bb_os_CopyDir(t_q,String(L"docs/monkeydoc/3rd party modules/",33)+t_t,true,false);
					}else{
						if(t_8==1){
							...
							if(t_9==String(L"png",3)){
								...
								if(t_9==String(L"monkeydoc",9)){
									CopyFile(t_q,String(L"docs/monkeydoc/3rd party modules/",33)+t_t);
								}
							}
						}
					}
			...
			if(t_7==1){
				...
				if(t_ext==String(L"monkey",6)){
					...
					String t_t2=t_dir+String(L"/",1)+t_name+String(L".monkeydoc",10);
					if(FileType(t_t2)==1){
						p_ParseMonkeydocFile(t_t2,t_q2);
						continue;
					}
					t_t2=t_dir+String(L"/monkeydoc/",11)+t_name+String(L".monkeydoc",10);
					if(FileType(t_t2)==1){
						p_ParseMonkeydocFile(t_t2,t_q2);
						continue;
					}
					p_ParseMonkeyFile(t_p2,t_q2);
				}
			}
		}
	}
	m_scopeTemplate=t_tmp;
}
...
void c_ApiDoccer::p_AddDecl(c_Decl2* t_decl,c_PageMaker* t_maker,c_Markdown* t_markdown){
	...
	if((t_eg).Length()!=0){
		...
		t_decl->m_docs->p_Set4(String(L"example",7),String(L"<pre>",5)+t_eg+String(L"</pre>",6));
		String t_file=t_decl->m_path.Replace(String(L".",1),String(L"_",1))+String(L".monkey",7);
		SaveString(t_eg,String(L"docs/html/examples/",19)+t_file);
		t_decl->m_docs->p_Set4(String(L"EXAMPLE_URL",11),String(L"examples/",9)+t_file);
...
void c_ApiDoccer::p_MakeScopeDocs(c_ScopeDecl* t_scope,c_PageMaker* t_maker){
	...
	if((t_desc).Length()!=0){
		...
		t_scope->m_docs->p_Set4(String(L"summary",7),t_summary);
		if(t_summary==t_desc){
			if(!((t_scope->m_docs->p_Get(String(L"example",7))).Length()!=0) && !((t_scope->m_docs->p_Get(String(L"links",5))).Length()!=0)){
				t_scope->m_docs->p_Set4(String(L"description",11),String());
			}
		}
	}
	t_maker->p_Clear();
...
void c_DocsDoccer::p_ParseDocs2(String t_dir,String t_indexcat){
	...
	while(t_2<t_.Length()){
		String t_f=t_[t_2];
		t_2=t_2+1;
		if(bb_os_ExtractExt(t_f)!=String(L"monkeydoc",9)){
			continue;
		}
		String t_docpath=bb_os_StripExt(t_f);
		m_george->p_AddPage(t_docpath,String());
		m_docs->p_Set4(t_docpath,t_dir+String(L"/",1)+t_f);
	}
}
...
String c_DocsDoccer::p_ResolveLink(String t_link,String t_text){
	if(t_link.StartsWith(String(L"../",3)) && t_link.EndsWith(String(L".monkey",7))){
		if(!((t_text).Length()!=0)){
			t_text=bb_os_StripDir(t_link);
		}
...
c_Parser* c_Parser::m_new(String t_src){
	m__toker=(new c_Toker)->m_new(t_src);
	p_Bump();
	return this;
}
...
void c_Decl2::p_Init(String t_kind,String t_ident,String t_type,String t_texts,String t_timpls,c_ScopeDecl* t_scope){
	...
	if((t_scope)!=0){
		...
		t_scope->m_declsByUident->p_Set2(m_uident,this);
	...
	m_docs->p_Set4(String(L"description",11),String());
	m_docs->p_Set4(String(L"example",7),String());
	m_docs->p_Set4(String(L"links",5),String());
	m_docs->p_Set4(String(L"params",6),String());
	m_docs->p_Set4(String(L"returns",7),String());
}
...
c_ScopeDecl::c_ScopeDecl(){
	m_declsByUident=(new c_StringMap2)->m_new();
	m_decls=(new c_Stack3)->m_new();
	m_declsByKind=(new c_StringMap3)->m_new();
	m_template=0;
}
c_Stack3* c_ScopeDecl::p_GetDecls(String t_kind){
	...
	if(!((t_decls)!=0)){
		...
		m_declsByKind->p_Set3(t_kind,t_decls);
	}
	return t_decls;
}
...
c_ModuleDecl* c_ModuleDecl::m_new(c_Decl* t_pdecl,c_ApiDoccer* t_doccer){
	...
	this->m_doccer=t_doccer;
	this->m_template=t_doccer->m_scopeTemplate;
	return this;
}
...
c_Decl2* c_ModuleDecl::p_FindDeclHere(String t_path){
	...
	if(!((t_decl)!=0)){
		t_decl=m_doccer->m_scopes->p_Get(String(L"monkey.lang",11))->p_FindDeclHere(t_path);
	}
	m_busy=false;
	return t_decl;
}
...
bool c_Map2::p_Set2(String t_key,c_Decl2* t_value){
	c_Node3* t_node=m_root;
	c_Node3* t_parent=0;
	int t_cmp=0;
	while((t_node)!=0){
...
c_NodeEnumerator6* c_Map2::p_ObjectEnumerator(){
	return (new c_NodeEnumerator6)->m_new(p_FirstNode());
}
c_Decl2* c_Map2::p_Get(String t_key){
...
c_Node3* c_Node3::m_new(String t_key,c_Decl2* t_value,int t_color,c_Node3* t_parent){
	this->m_key=t_key;
	this->m_value=t_value;
	this->m_color=t_color;
	this->m_parent=t_parent;
...
c_Stack3* c_Map3::p_Get(String t_key){
...
bool c_Map3::p_Set3(String t_key,c_Stack3* t_value){
	c_Node4* t_node=m_root;
	c_Node4* t_parent=0;
	int t_cmp=0;
	while((t_node)!=0){
...
c_NodeEnumerator5* c_Map3::p_ObjectEnumerator(){
...
c_Node4* c_Node4::m_new(String t_key,c_Stack3* t_value,int t_color,c_Node4* t_parent){
	this->m_key=t_key;
	this->m_value=t_value;
	this->m_color=t_color;
	this->m_parent=t_parent;
...
bool c_Map4::p_Set4(String t_key,String t_value){
...
c_NodeEnumerator3* c_Map4::p_ObjectEnumerator(){
	return (new c_NodeEnumerator3)->m_new(p_FirstNode());
}
String c_Map4::p_Get(String t_key){
...
c_StringMap4* c_Map6::p_Get(String t_key){
...
bool c_Map6::p_Set6(String t_key,c_StringMap4* t_value){
	c_Node7* t_node=m_root;
	c_Node7* t_parent=0;
	int t_cmp=0;
	while((t_node)!=0){
...
c_Node7* c_Node7::m_new(String t_key,c_StringMap4* t_value,int t_color,c_Node7* t_parent){
	this->m_key=t_key;
	this->m_value=t_value;
	this->m_color=t_color;
	this->m_parent=t_parent;
...
c_ClassDecl* c_ClassDecl::m_new(c_Decl* t_pdecl,c_ModuleDecl* t_scope){
	...
	this->m_template=t_scope->m_template;
	return this;
}
...
c_NodeEnumerator3* c_NodeEnumerator3::m_new(c_Node5* t_node){
	this->m_node=t_node;
	return this;
}
...
c_Node5* c_NodeEnumerator3::p_NextObject(){
	c_Node5* t_t=m_node;
	m_node=m_node->p_NextNode();
	return t_t;
}
...
String c_Markdown::p_ReplaceLinks(String t_src){
	...
	t_i=0;
	do{
		int t_i02=p_Find3(t_src,String(L"[",1),t_i);
		if(t_i02==-1){
			break;
		}
		...
		String t_t2=t_src.Slice(t_i02+1,t_i12);
		String t_p2=t_src.Slice(t_i12+2,t_i2);
		String t_r2=m__resolver->p_ResolveLink(t_p2,t_t2);
...
c_NodeEnumerator5* c_NodeEnumerator5::m_new(c_Node4* t_node){
	this->m_node=t_node;
	return this;
}
...
c_Node4* c_NodeEnumerator5::p_NextObject(){
	c_Node4* t_t=m_node;
	m_node=m_node->p_NextNode();
	return t_t;
}
...
c_NodeEnumerator6* c_NodeEnumerator6::m_new(c_Node3* t_node){
	this->m_node=t_node;
	return this;
}
...
c_Node3* c_NodeEnumerator6::p_NextObject(){
	c_Node3* t_t=m_node;
	m_node=m_node->p_NextNode();
	return t_t;
}
...
int bbMain(){
	...
	bb_os_DeleteDir(String(L"docs/monkeydoc/3rd party modules",32),true);
	CreateDir(String(L"docs/monkeydoc/3rd party modules",32));
...
int bbInit(){
	...
	c_Stack3::m_NIL=0;
	c_Stack6::m_NIL=0;
	c_George::m_MonkeyKeywords=String(L";Void;Strict;Public;Private;Property;Bool;Int;Float;String;Array;Object;Mod;Continue;Exit;Include;Import;Module;Extern;New;Self;Super;Eachin;True;False;Null;Not;Extends;Abstract;Final;Native;Select;Case;Default;Const;Local;Global;Field;Method;Function;Class;Interface;Implements;And;Or;Shl;Shr;End;If;Then;Else;Elseif;Endif;While;Wend;Repeat;Until;Forever;For;To;Step;Next;Return;Inline;Try;Catch;Throw;Throwable;Print;Error;Alias;",431).ToLower();
	return 0;
}
void gc_mark(){
}
